...
import { sanitizePostHtml } from "@/lib/htmlSanitizer";
import { fetchWpPostRawContent } from "@/lib/wpClient";
import { setLastReviewedMeta } from "@/lib/wpPostMeta";
import { upsertPersonPage } from "@/lib/personIndex";
import { pingIndexNow, isIndexNowConfigured } from "@/lib/indexNow";
import { buildPillarLinkHtml } from "@/lib/pillarPages";
import { isRecoveryActive } from "@/lib/pipelineConfig";
import {
  applyLightUpdate,
  fetchFreshNewsContext,
  generateLightUpdateSection,
} from "@/lib/lightUpdate";
...
export async function addInternalLinks(postId, postTitle, postLink, currentContent, religion, { useRandomFallback = true } = {}) {
  ...
  try {
    ...
    if (chosen.length === 0 && useRandomFallback) {
...
export async function createWordPressPost({
  ...
  let contentHtml = appendPostFooter(sanitizePostHtml(postContent || ""), publishedDate);
  const pillarHtml = buildPillarLinkHtml(answer?.religion);
  if (pillarHtml && !contentHtml.includes('pillar guide')) {
  ...
  const checklist = runPrePublishChecklist(postContentFinal, { seoTitle, isLightUpdate: false });

  if (!skipDuplicateCheck) {
    const { searchCelebrityUrl } = await import("@/lib/duplicateCheck");
  ...
  const internalLinks = await addInternalLinks(
    ...
    { useRandomFallback: !isRecoveryActive() }
  );
  ...
  await setLastReviewedMeta(postId, publishedDate);

  if (isIndexNowConfigured() && postData.link) {
    await pingIndexNow([postData.link]).catch(() => {});
  }
  ...
  if (websiteId) {
    try {
      await upsertPersonPage({
        ...
        lastReviewed: publishedDate.toISOString().slice(0, 10),
        canonicalSlug: slugVal,
      });
    } catch (error) {
      console.error("Error upserting person page:", error);
    }
  }
...
/**
 * Update an existing WordPress post (never changes slug).
 */
export async function updateWordPressPost({
  ...
  cookies,
  request,
  pipelineAction = "full-rewrite",
  existingRawContent = null,
}) {
  if (!postId) {
    throw new Error("postId is required for updateWordPressPost");
  }

  const celebrityName = title;
  const focusKeyword = answer?.religion ? `${title} ${answer.religion}` : `${title} religion`;
  const publishedDate = new Date();
  const isLight = pipelineAction === "light-update";

  let bodyHtml;
  if (isLight) {
    const existing =
      existingRawContent || (await fetchWpPostRawContent(postId)) || "";
    const freshContext = await fetchFreshNewsContext(celebrityName);
    const whyBlock = await generateLightUpdateSection(celebrityName, freshContext);
    bodyHtml = applyLightUpdate(sanitizePostHtml(existing), whyBlock);
  } else {
    bodyHtml = sanitizePostHtml(postContent || "");
    bodyHtml = appendPostFooter(bodyHtml, publishedDate);
    const pillarHtml = buildPillarLinkHtml(answer?.religion);
    if (pillarHtml && !bodyHtml.includes("pillar guide")) {
      bodyHtml = bodyHtml + "\n" + pillarHtml;
    }
  }
  ...
  const postRes = await fetch(`${WP_BASE}/posts/${postId}?_fields=link,slug`, {
    headers: { Authorization: WP_AUTH_HEADER },
  });
  const existingPost = postRes.ok ? await postRes.json() : {};
  const postUrl = existingPost.link || null;
  ...
  const postContentFinal = bodyHtml + jsonLdScript;

  const checklist = runPrePublishChecklist(postContentFinal, {
    seoTitle,
    isLightUpdate: isLight,
  });
  ...
  const postResponse = await fetch(`${WP_BASE}/posts/${postId}`, {
    method: "POST",
    headers: {
      Authorization: WP_AUTH_HEADER,
      "Content-Type": "application/json",
  ...
  if (!postResponse.ok) {
    const errorText = await postResponse.text();
    throw Object.assign(
      new Error(`Failed to update post ${postId}: ${postResponse.status} ${errorText.substring(0, 300)}`),
      { statusCode: postResponse.status }
    );
  }
  ...
  if (websiteId) {
    try {
      await upsertPersonPage({
        ...
        postUrl: postData.link || null,
        content: postContentFinal,
        slug: existingPost.slug || null,
        metaDescription,
        religion: answer?.religion || null,
        wpStatus: "publish",
        contentQuality: isLight ? "good" : "unknown",
        lastReviewed: publishedDate.toISOString().slice(0, 10),
        canonicalSlug: existingPost.slug || null,
      });
    } catch (error) {
      console.error("Error upserting person page after update:", error);
    }
  }
