...
// Only matched against H2/H3 text — not full body (e.g. "their religion" in
// normal prose is fine; unfilled template placeholders in headings are not).
const BANNED_HEADING_PATTERNS = [
  /comparisons with other celebrities on religion/i,
  /views on faith/i,
  /religious beliefs and practices/i,
  /life partner'?s religion/i,
  /\btheir\b\s*(religion|faith)\b/i,
  /\bthey'?re\b.*\b(religion|faith)\b/i,
  /\[name\]/i,
  /\btheir\b$/i,
];
...
export function runPrePublishChecklist(contentHtml, { seoTitle, isLightUpdate = false } = {}) {
  const failures = [];

  if (isLightUpdate) {
    if (!contentHtml.includes("PIPELINE:why-in-news:start")) {
      failures.push("Light update missing why-in-news marker block");
    }
    const inlineLinkCount = (contentHtml.match(/<a\s+href=/gi) || []).length;
    if (inlineLinkCount < 1) {
      failures.push(`Light update needs at least 1 source link (${inlineLinkCount})`);
    }
  ...
  if (!/what religion is/i.test(seoTitle || "")) {
    failures.push("Title should state the religion answer (What Religion Is …)");
  }

  const headingText = [
    ...(contentHtml.match(/<h[23][^>]*>[\s\S]*?<\/h[23]>/gi) || []),
  ]
    .map((h) => h.replace(/<[^>]+>/g, " "))
    .join("\n");
  ...
  for (const pattern of BANNED_HEADING_PATTERNS) {
    if (pattern.test(headingText)) {
      failures.push(`Banned heading pattern: ${pattern}`);
    }
  }
  ...
  if (!/"@type"\s*:\s*"FAQPage"/i.test(contentHtml)) {
    failures.push("Missing FAQPage JSON-LD schema");
  }
