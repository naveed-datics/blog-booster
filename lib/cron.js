...
export function initializeCronJob() {
  ...
  // Get base URL for API calls - check multiple sources for Vercel compatibility
  ...
  if (!baseUrl) {
    ...
    if (process.env.VERCEL_PROJECT_PRODUCTION_URL) {
      ...
      // VERCEL_URL is the deployment URL (e.g., project-abc123.vercel.app)
      baseUrl = `https://${process.env.VERCEL_URL}`;
    } else if (process.env.NODE_ENV === 'production') {
      // In production without Vercel vars, log a warning
      console.warn('[Cron] ⚠️ No NEXT_PUBLIC_BASE_URL or VERCEL_URL set in production!');
      baseUrl = null; // Will skip cron execution
    } else {
  ...
  if (!baseUrl) {
    console.error('[Cron] ❌ Cannot initialize cron job without a valid base URL. Set NEXT_PUBLIC_BASE_URL environment variable.');
    return;
  }
